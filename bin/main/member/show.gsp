        ...
        <div class="member-header">
            <div class="row align-items-center">
                ...
                <div class="col-md-4 text-end">
                    <g:switch value="${member?.status}">
                        <g:when test="${member?.status == 'ACTIVE'}">
                            <span class="badge bg-success fs-6 px-3 py-2">ACTIVE</span>
                        </g:when>
                        <g:when test="${member?.status == 'PENDING'}">
                            ...
                            <g:each var="row" in="${0..<((matrixViz?.grid?.size() ?: 0) as int)}">
                                <div class="col-12">
                                    <div class="d-flex g-2 justify-content-center">
                                        <g:each var="cellIdx" in="${0..<((matrixViz?.grid?.get(row)?.size() ?: 0) as int)}">
                                            <g:set var="cell" value="${matrixViz?.grid?.get(row)?.get(cellIdx)}" />
